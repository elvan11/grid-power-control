...
- [x] **SolisCloud provider module (Edge Functions)**
  ...
  - Status update (2026-02-21): added `provider_connection_get` Edge Function for authenticated retrieval of saved provider connection details in app settings so existing encrypted credentials can repopulate editable Connect Cloud Service fields during QA verification.
  ...
  - Status update (2026-02-19): added post-control read-back verification polling for each CID and per-CID full procedure retries (up to 3 total attempts); `provider_response.attempts` now includes procedure/verification attempt metadata for full traceability in `control_apply_log`.
  - Status update (2026-02-19): started Edge Function unit-test coverage with mocked Solis API responses in `supabase/functions/_shared/solis_test.ts` (covers signed `atRead` calls, full happy-path control flow for CIDs `5035`/`5041`, and peak-shaving validation bounds/step enforcement).
  - Status update (2026-02-19): redesigned Solis provider Edge Functions to use injectable handler modules for unit testing (`provider_apply_control/handler.ts`, `provider_connection_test/handler.ts`, `provider_connection_upsert/handler.ts`), while keeping `index.ts` files as thin production wiring layers.
...
- [x] **Responsive layout (tablet + desktop web)**
  ...
  - Status update (2026-02-11): refined responsive schedule/navigation UI details by increasing left `NavigationRail` label typography (`app/lib/core/widgets/gp_bottom_nav.dart`), removing day-chip checkmark glyphs so M/T/W/T/F/S/S chip widths stay stable with color-only selection (`app/lib/features/schedules/schedules_page.dart`), and moving daily schedule edit to a top-right pen icon on each card to match installation tiles (`app/lib/features/schedules/schedules_page.dart`).
  - Status update (2026-03-12): removed global top/bottom page padding from responsive scaffold spacing and kept only horizontal gutters so scrollable pages sit flush against top/bottom edges; updated sign-in to render the background image edge-to-edge while preserving inner form padding via list content insets (`app/lib/core/widgets/gp_responsive.dart`, `app/lib/core/widgets/gp_scaffold.dart`, `app/lib/features/auth/sign_in_page.dart`).
- [ ] **Plant sharing**
...
- [ ] **Deployment**
  ...
  - Status update (2026-02-21): web deploy workflows now pass `APP_BUILD_DATE` and `APP_BUILD_NUMBER` as Dart defines. Build number is computed per UTC day (increments for each build and resets to `1` each new UTC date) so Settings → About shows the expected date-based build stamp.
